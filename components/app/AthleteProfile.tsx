...
import {
  AthleteOverviewStage,
  IMMERSIVE_OVERLAY_CARD,
} from "@/components/app/AthleteOverviewStage";
import { buildAthleteHudColumns } from "@/components/app/AthleteProfileHud";
...
export function AthleteProfile({ athlete }: { athlete: Athlete }) {
  ...
  const hudColumns = buildAthleteHudColumns(measurables, wearables, attendedCount);
    ...
    <>
      <Card>
        ...
        <CardContent className="pt-0">
          <p className="text-sm leading-snug text-pitch">{athlete.ai_summary}</p>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            <p className="text-sm text-slate">
              Recommend introducing to Coach Mike Chen — Sun Devil State University
              (active scout viewing your data).
            </p>
            <Link
  ...
  const otherTabPanels = (
    <>
      <TabsContent value="measurables" className="mt-0 space-y-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="mb-4 text-sm">
              {attendedCount} sessions attended · 90-day calendar
            </p>
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-start gap-6">
          ...
          <div>
            ...
            <div className="mt-2 flex gap-2">
              <Badge variant={athlete.status === "at_risk" ? "danger" : "success"}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <div className="mt-3">
          <TabsContent value="overview" className="mt-0">
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,7fr)_minmax(0,3fr)]">
              <AthleteOverviewStage
                ...
                leftMetrics={hudColumns.left}
                rightMetrics={hudColumns.right}
              >
                <Card className={cn(IMMERSIVE_OVERLAY_CARD, "w-full max-w-xl")}>
                  <CardContent className="px-3 py-2.5">
                    <p className="mb-2 text-center text-[10px] font-bold uppercase tracking-[0.12em] text-orange">
                      Recent measurables
                    </p>
                    <div className="flex flex-nowrap items-center justify-center gap-1.5 sm:gap-2">
                      {measurables.slice(-4).map((m) => (
                        <div
                          key={m.id}
                          className="min-w-0 shrink-0 rounded-md bg-chalk/80 px-2 py-1.5 text-center sm:px-2.5"
                        >
                          <p className="truncate text-[9px] font-medium uppercase tracking-wide text-slate sm:text-[10px]">
                            {m.metric.replace("_", " ")}
                            {m.is_pr && " · PR"}
                          </p>
                          <p className="whitespace-nowrap text-xs font-bold leading-tight sm:text-sm">
                            {m.value} {m.unit}
                          </p>
                        </div>
                      ))}
          ...
          <div
            className={cn(
              "grid gap-6 lg:grid-cols-[minmax(0,7fr)_minmax(0,3fr)]",
              activeTab === "overview" && "hidden"
            )}
          >
            <div className="space-y-4">{otherTabPanels}</div>
            <div className="space-y-4">{sidebar}</div>
          </div>
        </div>
      </Tabs>
